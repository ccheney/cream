 ...
 */
function calculateChange(current: number | null, previous: number | undefined): QuoteChange | null {
...
function HeaderRow({ symbol, quote, change }: HeaderRowProps) {
	return (
...
function BidAskDepth({ quote, showDepth }: DepthInfoProps) {
	if (!showDepth || quote.bid == null || quote.ask == null) {
		return null;
	}
...
interface DayRangeSectionProps {
	quote: Quote;
}

function DayRangeSection({ quote }: DayRangeSectionProps) {
	if (quote.high == null || quote.low == null || quote.last == null) {
		return null;
	}
...
function VolumeSection({ quote, volumePercent }: VolumeSectionProps) {
	return (
...
export const EnhancedQuoteHeader = memo(function EnhancedQuoteHeader({
	...
	const regimeColor = useMemo(() => getRegimeClass(regime), [regime]);
	...
	return (
		...
		>
			<div className="flex items-start justify-between">
				<HeaderRow symbol={symbol} quote={quote} change={change} />
				{regime && (
			...
			<BidAskDepth quote={quote} showDepth={showDepth} />

			<div className="flex items-center gap-6">
				<DayRangeSection quote={quote} />
				<VolumeSection quote={quote} volumePercent={volumePercent} />
			</div>
		</div>
	);
});
