 ...
 * Repository supports: momentum, trend, volatility, volume, sentiment
 * Hypothesis adds: liquidity, correlation, microstructure, regime
 */
function mapToRepositoryCategory(
	hypothesisCategory: string
): "momentum" | "trend" | "volatility" | "volume" | "sentiment" {
	const directMappings = ["momentum", "trend", "volatility", "volume", "sentiment"] as const;
	...
	// Extended categories (liquidity, correlation, microstructure, regime) map to sentiment
	return "sentiment";
}
...
export const initiatePaperTradingStep = createStep({
	...
	execute: async ({ inputData }) => {
		...
		try {
			...
			// Create indicator record with 'staging' status (UUIDv7 generated by database)
			const indicator = await indicatorsRepo.create({
