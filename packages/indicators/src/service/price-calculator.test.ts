...
function buildEmaVsSmaBars(): OHLCVBar[] {
	const bars: OHLCVBar[] = [];
...
describe("PriceCalculatorAdapter.calculate basic behavior", () => {
...
describe("PriceCalculatorAdapter.calculate with sufficient data", () => {
	test("calculates all indicators", () => {
		const adapter = new PriceCalculatorAdapter();
...
describe("Moving Averages ordering", () => {
...
describe("Moving Averages responsiveness", () => {
	test("EMA is more responsive than SMA", () => {
		const adapter = new PriceCalculatorAdapter();
		const bars = buildEmaVsSmaBars();
		const result = adapter.calculate(bars);
		const currentPrice = requireValue(bars.at(-1), "last bar").close;
...
describe("Bollinger Bands", () => {
	...
	test("percentB is 0-1 when price is within bands", () => {
		const adapter = new PriceCalculatorAdapter();
		const bars = generateBars(100, 100, 0.01);
		const result = adapter.calculate(bars);
...
describe("Momentum", () => {
	...
	test("requires sufficient bars for each period", () => {
		...
		expect(result.momentum_1m).not.toBeNull();
		expect(result.momentum_3m).toBeNull();
		expect(result.momentum_6m).toBeNull();
		expect(result.momentum_12m).toBeNull();
	});
});
...
describe("Volatility", () => {
	...
	test("higher volatility for more volatile data", () => {
		const adapter = new PriceCalculatorAdapter();
		const lowVolBars = generateBars(50, 100, 0.005);
		const highVolBars = generateBars(50, 100, 0.03);
	...
	test("volatility is annualized", () => {
		const adapter = new PriceCalculatorAdapter();
		const bars = generateBars(50, 100, 0.01);
		const result = adapter.calculate(bars);

		expect(result.realized_vol_20d).not.toBeNull();
		expect(requireValue(result.realized_vol_20d, "realized_vol_20d")).toBeGreaterThan(0.05);
		expect(requireValue(result.realized_vol_20d, "realized_vol_20d")).toBeLessThan(0.5);
	});
});

describe("PriceCalculatorAdapter integration", () => {
