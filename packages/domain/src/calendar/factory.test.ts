...
describe("createCalendarService", () => {
	...
	it("error message includes environment variable name", async () => {
		delete Bun.env.ALPACA_KEY;
		delete Bun.env.ALPACA_SECRET;
		await expect(createCalendarService({ mode: "PAPER" })).rejects.toMatchObject({
			missingVar: "ALPACA_KEY",
			mode: "PAPER",
		});
	});
	...
	it("throws CalendarConfigError when only ALPACA_KEY is present", async () => {
		...
		delete Bun.env.ALPACA_SECRET;
		await expect(createCalendarService({ mode: "PAPER" })).rejects.toMatchObject({
			missingVar: "ALPACA_SECRET",
		});
	});
});

describe("singleton management", () => {
	it("clears previously set singleton after reset", () => {
		const sentinel = {
	...
	it("requireCalendarService throws if not initialized", () => {
		expect(() => requireCalendarService()).toThrow("not initialized");
	});

	it("resetCalendarService does not throw if not initialized", () => {
...
describe("CalendarConfigError", () => {
	it("has correct name", () => {
		expect(new CalendarConfigError("ALPACA_KEY", "PAPER").name).toBe("CalendarConfigError");
	});
