...
describe("GrpcError constructor", () => {
	...
	test("marks retryable errors correctly", () => {
		expect(new GrpcError("unavailable", "UNAVAILABLE").retryable).toBe(true);
		expect(new GrpcError("rate limited", "RESOURCE_EXHAUSTED").retryable).toBe(true);
		expect(new GrpcError("timeout", "DEADLINE_EXCEEDED").retryable).toBe(true);
		expect(new GrpcError("bad request", "INVALID_ARGUMENT").retryable).toBe(false);
		expect(new GrpcError("not found", "NOT_FOUND").retryable).toBe(false);
	});
});

describe("GrpcError.fromConnectError", () => {
	test("handles connect errors", () => {
		const connectError = {
			code: "unavailable",
...
describe("GrpcError helper methods", () => {
	test("work correctly", () => {
		...
		expect(new GrpcError("rate limited", "RESOURCE_EXHAUSTED").isRateLimited()).toBe(true);
		expect(new GrpcError("timeout", "DEADLINE_EXCEEDED").isTimeout()).toBe(true);
		expect(new GrpcError("invalid", "INVALID_ARGUMENT").isInvalidInput()).toBe(true);
	});
});
...
describe("RetryBackoff delays", () => {
	...
	test("applies jitter when factor > 0", () => {
		const delays: number[] = [];
		for (let i = 0; i < 10; i++) {
			const backoff = new RetryBackoff({ baseDelayMs: 1000, jitterFactor: 0.5 });
