...
// ============================================
// Databento Timestamp Utilities
// ============================================

/**
 * Databento provides 4 timestamps per event:
 *
 * 1. ts_event - Exchange timestamp (when event occurred)
 * 2. ts_recv - Databento receive timestamp (when they received it)
 * 3. ts_in_delta - Time between exchange and Databento receive
 * 4. ts_out - Our processing timestamp
 *
 * Use ts_event for:
 * - Order book analysis
 ...
 */
export interface DatabentoTimestamps {
  /** Exchange timestamp (primary for analysis) */
  ts_event: string;
  /** Databento receive timestamp */
  ts_recv: string;
  /** Delta between exchange and receive (nanoseconds) */
  ts_in_delta?: number;
...
/**
 * Select appropriate Databento timestamp based on use case
 *
 * @param timestamps - Databento timestamp set
 * @param useCase - What the timestamp will be used for
 * @returns The appropriate timestamp
 */
export function selectDatabentoTimestamp(
  ...
  switch (useCase) {
    case "analysis":
      // Use exchange timestamp for historical analysis
      return timestamps.ts_event;
    case "latency":
      // Use receive timestamp for latency calculations
      return timestamps.ts_recv;
    case "processing":
      // Use our processing timestamp
      return timestamps.ts_out;
  }
}

/**
 * Calculate Databento data latency
 *
 * @param timestamps - Databento timestamp set
 * @returns Latency in milliseconds
 */
export function calculateDatabentoLatency(timestamps: DatabentoTimestamps): {
  exchangeToRecvMs: number;
  recvToProcessMs: number;
  totalMs: number;
} {
  const eventTime = new Date(timestamps.ts_event).getTime();
  const recvTime = new Date(timestamps.ts_recv).getTime();
  const outTime = new Date(timestamps.ts_out).getTime();

  return {
    exchangeToRecvMs: recvTime - eventTime,
    recvToProcessMs: outTime - recvTime,
    totalMs: outTime - eventTime,
  };
}
