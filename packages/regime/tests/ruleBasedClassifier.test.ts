...
function createTrendingOHLCVBars(
	...
	const change = direction === "up" ? 0.01 : -0.01;

	for (let i = 0; i < count; i++) {
		price = price * (1 + change);
		candles.push(
			createOHLCVBar(price, price * 1.005, price * 0.995, 1000000, Date.now() + i * 3600000),
		);
	}
...
function createHighVolatilityOHLCVBars(startPrice: number, count: number): OHLCVBar[] {
	...
	for (let i = 0; i < count; i++) {
		...
		price = price * (1 + change);
		candles.push(
			createOHLCVBar(price, price * 1.03, price * 0.97, 2000000, Date.now() + i * 3600000),
		);
	}
...
describe("classifyRegime trends", () => {
...
describe("classifyRegime volatility", () => {
	...
	test("classifies high volatility correctly", () => {
		...
		const historicalAtr = normalOHLCVBars.map((candle) => candle.high - candle.low);
...
describe("classifyRegime configuration and metrics", () => {
...
describe("classifyRegime edge cases basic", () => {
...
describe("classifyRegime edge cases extreme movements", () => {
