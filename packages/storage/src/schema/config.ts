...
export const tradingConfig = pgTable(
	"trading_config",
	{
		...
		totalConsensusTimeoutMs: integer("total_consensus_timeout_ms").default(
			300000,
		),
		...
		kellyFraction: numeric("kelly_fraction", { precision: 4, scale: 3 }).default(
			"0.5",
		),

		// Schedule (milliseconds)
		tradingCycleIntervalMs: integer("trading_cycle_interval_ms").default(
			3600000,
		),
		predictionMarketsIntervalMs: integer(
			"prediction_markets_interval_ms",
		).default(900000),

		// Global LLM model selection
		globalModel: text("global_model")
			.notNull()
...
export const agentConfigs = pgTable(
	...
	(table) => [
		...
		uniqueIndex("idx_agent_configs_env_agent").on(
			table.environment,
			table.agentType,
		),
	],
...
export const universeConfigs = pgTable(
	...
	(table) => [
		...
		index("idx_universe_configs_env_status").on(
			table.environment,
			table.status,
		),
...
export const constraintsConfig = pgTable(
	"constraints_config",
	{
		...
		maxNotional: numeric("max_notional", { precision: 14, scale: 2 })
			.notNull()
			.default("50000"),
		maxPctEquity: numeric("max_pct_equity", { precision: 4, scale: 3 })
			.notNull()
			.default("0.1"),
		...
		maxConcentration: numeric("max_concentration", { precision: 4, scale: 3 })
			.notNull()
			.default("0.25"),
		maxCorrelation: numeric("max_correlation", { precision: 4, scale: 3 })
			.notNull()
			.default("0.7"),
		maxDrawdown: numeric("max_drawdown", { precision: 4, scale: 3 })
			.notNull()
			.default("0.15"),

		// Options greeks limits
		maxDelta: numeric("max_delta", { precision: 8, scale: 2 })
			.notNull()
			.default("100"),
		maxGamma: numeric("max_gamma", { precision: 8, scale: 2 })
			.notNull()
			.default("50"),
		maxVega: numeric("max_vega", { precision: 10, scale: 2 })
			.notNull()
			.default("1000"),
		maxTheta: numeric("max_theta", { precision: 10, scale: 2 })
			.notNull()
			.default("500"),
